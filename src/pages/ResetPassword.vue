<template>

    <q-page class="flex window-width" id="page">
      <div class="row justify-center items-center window-width " >
        <div class="col-md-3 col-xs-10 bg-white rounded-borders" style="height:60vh">
          <div class="row flex full-height">

            <div class="col-md-12 col-xs-12 flex">
              <div class="justify-center flex full-width items-center">

                <q-form
                  @submit="ResetPassword"
                  class="q-gutter-md full-width"
                  style="margin-right: 1rem !important;"
                >
                <div  class="text-center"><q-img
                  src="numekoacademylogo.png"

                  spinner-color="primary"
                  spinner-size="82px"
                /></div>
                <q-input v-model="newPassword"  type="password" label="Yeni Şifreniz" filled outlined input-class="input-text" />
                <q-input v-model="passwordAgain"  type="password" label="Şifrenizi Tekrar Giriniz" filled outlined input-class="input-text" />
                  <div class="flex justify-center">
                    <q-btn label="Şifremi Değiştir" type="submit" color="primary"/>
                  </div >
                  <div class="row flex text-center">
                    <div class="col-12 cursor" @click="$router.push('/')">Giriş Yap</div>

                  </div>

                </q-form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </q-page>
</template>
<script>
import { ref } from 'vue'
import { useQuasar } from 'quasar'
import { api } from 'src/boot/axios'
import { useRoute, useRouter } from 'vue-router'
export default {
  setup () {
    const route = useRoute()
    const router = useRouter()
    const $q = useQuasar()
    const newPassword = ref(null)
    const passwordAgain = ref(null)
    const showNotify = (message, type) => {
      $q.notify({
        message,
        color: type,
        position: 'top-right'
      })
    }
    const ResetPassword = () => {
      let token = route.query.resetToken.toString()
      console.log(token)

      token = token.replaceAll(' ', '+')
      if (newPassword.value === passwordAgain.value) {
        api.post('Account/PasswordReset', {
          token,
          email: route.query.email,
          newPassword: newPassword.value,
          checkPassword: passwordAgain.value
        }).then(() => {
          showNotify('Şifre Değiştirme İşlemi Gerçekleşti', 'positive')
          showNotify('Giriş Ekranına Yönlendiriliyorsunuz.', 'positive')
          setTimeout(() => {
            router.push('/login')
          }, 1000)
        }).catch(({ response }) => {
          response.data.forEach(element => {
            showNotify(element, 'negative')
          })
        })
      } else {
        $q.notify({
          message: 'Şifreleriniz Birbiri ile Uyuşmuyor.',
          color: 'negative',
          position: 'top-right'
        })
      }
    }
    return {
      newPassword, passwordAgain, ResetPassword, showNotify
    }
  },
  async mounted () {
  },
  computed: {
    isMobile () {
      return this.$q.platform.is.mobile
    }
  }
}
</script>
<style scoped>
.login-background{
  background: linear-gradient(-135deg,#58bfff,#adfffc);
}
@media (min-width:1000px) {
  .input-text{
    width:18rem !important;
}
@media (max-width:998px) {
  .input-text{
    width:70vw !important;
  }
}
}
.cursor{
  cursor:pointer;
}
</style>
